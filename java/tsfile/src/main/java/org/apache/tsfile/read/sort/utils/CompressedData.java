package org.apache.tsfile.read.sort.utils;
...
public class CompressedData {
    ...
    public CompressedData(ByteBuffer v, int count) {
        vals = new byte[v.limit()-v.position()-4-(count+3)/4];
        lens = new byte[(count+3)/4];
        v.get(vals,0,vals.length);
        v.get(lens,0,lens.length);
    }
